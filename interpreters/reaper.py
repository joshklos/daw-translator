...
from interpreters.helpers import *


class ReaperInt(object):

    def __init__(self, version="4.76/OSX64", version_num="4.76"):
    ...
    def write(self, session, debug):
        myOutput = '<REAPER_PROJECT 0.1 "4.76/OSX64"\n'
        myOutput += '\tRIPPLE 0\n\tGROUPOVERRIDE 0 0 0\n\tAUTOXFADE 1\n\tENVATTACH 1\n'
        myOutput += '\tMIXERUIFLAGS 11 48\n\tPEAKGAIN 1\n\tFEEDBACK 0\n\tPANLAW 1\n'
        myOutput += '\tPROJOFFS 0 0\n\tMAXPROJLEN 0 600\n\t'
        for track in session.tracks:
            myOutput += "\t<TRACK \n"
            myOutput += "\t\t"
            myOutput = myOutput + "NAME " + track.name + "\n"
            for item in track.items:
                myOutput = myOutput + "\t\t\t<ITEM\n"
                myOutput = myOutput + "\t\t\tPOSITION " + item.startTime + "\n"
                myOutput = myOutput + "\t\t\tLENGTH " + item.length + "\n"
                myOutput = myOutput + "\t\t\tNAME " + item.name + "\n"
                myOutput = myOutput + "\t\t\t<SOURCE " + item.source_file.fileType() + '\n FILE "' + item.source_file.fpath + '" 1\n>'
            myOutput = myOutput + "\t\t>"
        myOutput = myOutput + ">"
        return myOutput
